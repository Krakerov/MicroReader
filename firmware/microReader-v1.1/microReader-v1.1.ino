...
void drawPage(File file) {                          // Отрисовка страницы на олед
  ...
  while (!oled.isEnd() && file.available() && Grp<3) {       // Выводим символы пока не кончился дисплей или файл
    wort = file.read();
    if (wort == '$') {
      Grp = Grp+1;
    }
    if (Grp>=3){
      ...
      oled.write(wort);}                        // Транслируем символы в дисплей
  } oled.update();                                  // Выводим картинку
}
